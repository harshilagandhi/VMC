...
public class expense_add extends AppCompatActivity {
	ImageView datetime,odometer_icon,expense_type,location_icon,reason_icon,note_icon,icon_add_category,expense_icon;
	TextView date,time;
...
//		intialize
		...
		todaydate=new StringBuilder().append(tday).append("/").append(tmonth).append("/").append(tyear).toString();
			...
			public void onClick(View v) {
				...
				datePickerDialog=new DatePickerDialog(expense_add.this, new DatePickerDialog.OnDateSetListener() {
					...
					public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
						date.setText(dayOfMonth+"/"+(month+2)+"/"+year);
					}
				},date_,month_,year_);
...
//		set on click listener for add the category addition
		...
		ArrayList<String> expense_list=new ArrayList<String>();
